...
allowed: `asksPerDay: 0` refuses every ask. Unknown fields are refused with 400
`invalid_input`.
...
- access control: members and group mappings, MCP keys, the access mode, and `disable` and
  `enable`, so access can always be revoked or tightened
...
branding. Nothing else is disclosed. The portal list (`GET /api/tenants`) still shows the
portal with its status, and cross-portal asks leave it out. Scheduled syncs, watches and
enrichment runs stop.
...
saving changes. It counts research asks (`POST /api/t/:slug/ask`), help-assistant asks
(`POST /api/t/:slug/docs/ask`) and the MCP `answer_question` tool. A cross-portal ask counts
once on each portal it reaches, and is admitted on every selected portal or on none. An ask is
counted when it is admitted, before its answer streams, so a refused ask costs nothing
upstream. A refused MCP call returns the same body as a tool error.
...
- `bytes` is the source content the portal has written to its knowledge box: file bytes for
  ...
  pages. A link that is handed to the platform crawler carries no content from the portal and
  counts as zero bytes. The platform does not report stored source size (its counters report
  index size, which is a different quantity), so the portal keeps this ledger itself. The
  ledger starts the first time the portal observes its knowledge box, and it tracks each
  resource the portal adds or deletes. `bytes` is `null` whenever the box holds a resource the
  ledger cannot size: content that was there before the ledger started, content added to the
  box outside CorpusKit, or a write whose outcome could not be sized. Deleting such a resource
  through the portal brings the count back. Content removed outside CorpusKit is not noticed,
...
- `lastActivityAt` is the time of the most recent ask or other successful write on the portal,
  such as saved research or a content change. Reads are not recorded, because CorpusKit never
  writes state on a read, so browsing and searching alone do not move it. It is recorded at
  most once a minute.
...
2. After authorisation it applies read-only mode to every portal route that
   ...
   ask, research and access-control permissions, and the access-mode, `disable` and `enable`
   routes. The declaration test lists the exemptions independently. A caller without the
   permission still gets 401 or 403, not 423.
3. The knowledge-box management provider is wrapped so every write rechecks the status at the
   moment it happens. The same wrapper admits resource and byte usage and refuses agent
...
4. Ask routes and the MCP `answer_question` tool count each ask before calling the platform.
...
- A portal administrator can still disable or enable a read-only portal.
...
same records as JSON files under `DATA_DIR/lifecycle/`. A record that cannot be read fails
closed: the request fails rather than treating the portal as active or unlimited. Deleting a
portal removes its records, so a later portal with the same slug starts fresh.
...
- A suspended portal shows a paused screen in its own branding, to signed-in and anonymous
  visitors alike.
- The upload, link and text forms explain a resource limit, a storage limit or unavailable
  usage in plain words, and keep what was entered. An ask over the daily limit explains when
  the limit resets, and it does not retry automatically. A refused agent run explains that
